...
\begin{document}
	...
	\begin{linenumbers}
		...
		Sticky Pis can be fitted with cages to prevent small vertebrates to predate trapped insects.
        ...
        The use of a inferior optics and infrared lighting resulted in lower quality images where insects were often blurry.
        However, in this simplified scenario, there were no occlusion, and the classification was binary (fly \emph{v.s.} background).
        Therefore, we used a simplified approach: we trained and applied independent Mask-RCNN to segment flies from their background. 
		...
		Therefore, tracking (using the Siamese Insect Matcher) likely improves recall as insects that are missed on some frames may be detected on previous or subsequent ones.
		
		
		Another limitation of our device is the sparsity of the images taken (every 20 minutes). 
		According to their context and questions, users can program the hardware timer with a different interval.
		However, we judged three-times-per-hour a good compromise between the time resolution -- an hourly resolution being necessary to study chronobiology or the impact of fast weather variations -- and battery and data storage efficiency.
		Furthermore, a denser use of the flash light (\emph{e.g.}, every minute) may disturb wildlife\cite{longcore_ecological_2004}.		
		...
		Indeed, specimens can be used for \emph{a posteriori} visual and DNA-based identification, which would not be possible if animals were released or kept in a common container. Furthermore, trapping insects permanently greatly reduced the risk of recapturing several times the same individuals.
		...
		This study was performed at the level of a small agricultural field, where the local abiotic variations were minor (see Methods section and reported abiotic conditions in Supplementary Fig.~\ref{supfig:08}). 
   ...
   \begin{supfig}[ht]
	...
	The average between-device standard deviation for hourly temperature and humidity reads was, respectively, 0.68°C and 3.95\% for the raspberry field (A) and 0.63°C and 3.4\% for the blackberry experiment (B).
   }
	\label{supfig:08}
	\end{supfig}
