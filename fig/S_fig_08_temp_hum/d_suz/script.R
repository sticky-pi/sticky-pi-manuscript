...
p3 <- ggplot(na.omit(sunset_rise), aes(xmin=sunrise, xmax=sunset, ymin=10, ymax=5)) +
      geom_rect(fill="white", colour=NA) +
      geom_rect(data= na.omit(sunset_rise)[1:.N-1], aes(xmin=sunset, xmax=sunrise_n, ymin=10, ymax=5), fill="black", colour=NA)
...
dev_temp_hour_dt = na.omit(temp_dt[, .(temp = mean(temp),
