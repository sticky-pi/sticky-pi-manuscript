...
%DIF DEL initial_submission.tex   Thu Mar 24 05:33:34 2022
%DIF ADD main.tex                 Fri Apr 29 08:52:16 2022
...
\title{Sticky Pi, a high-frequency smart trap to study insect circadian activity under natural conditions}
...
\begin{document}
	...
	\begin{linenumbers}
		...
		\DIFaddbegin \DIFadd{Sticky Pis can be fitted with cages to prevent small vertebrates from predating trapped insects.
		}\DIFaddend Another unique feature is their camera-triggered backlit flashlight, which enhances the contrast, reduces glare and allows for night-time imaging. \DIFaddbegin \DIFadd{Most sticky cards available on the market are thin and translucent, which allows for the transmission of light. 
		...
		To segment “insects” from their “background”, we based the Universal Insect Detector on Mask R-CNN \cite{he_mask_2017}, and trained it on \DIFdelbegin \DIFdel{239 }\DIFdelend \DIFaddbegin \DIFadd{240 }\DIFaddend hand-annotated images \DIFdelbegin \DIFdel{of trapped insects }\DIFdelend \DIFaddbegin \DIFadd{from Sticky Pis as well as  120 ``foreign'' images acquired with different devices }\DIFaddend (see Methods section).
		On the validation dataset, our algorithm had an overall \DIFdelbegin \DIFdel{69.6}\DIFdelend \DIFaddbegin \DIFadd{82.9}\DIFaddend \% recall and \DIFdelbegin \DIFdel{90.5}\DIFdelend \DIFaddbegin \DIFadd{91.2}\DIFaddend \% precision (Supplementary Fig.~S\ref{supfig:02}). Noticeably, recall increased to \DIFdelbegin \DIFdel{79.3}\DIFdelend \DIFaddbegin \DIFadd{90.5}\DIFaddend \% when excluding the 25\% smallest objects (area <~1000 px. \emph{i.e.}, 2.12~mm²), indicating that the smallest insect instances are ambiguous.
		\DIFaddbegin \DIFadd{When performing validation on the foreign dataset of 20 images acquired with  the Raspberry Pi camera HQ, we obtained a precision 96.4\% and a recall of 92.2\%, indicating that newly available optics may largely increase segmentation performance (though, all experimental data in this study were obtained with the original camera, before the HQ became available).
		}\DIFaddend 
		...
		\DIFaddbegin \DIFadd{The DD and LL experiments were performed independently, and each compared to their own internal LD control.
        The use of a infrared optics and lighting resulted in lower quality images (}\emph{\DIFadd{i.e.}} \DIFadd{reduced sharpness).
        However, in this simplified scenario, there were no occlusions, and the classification was binary (fly }\emph{\DIFadd{v.s.}} \DIFadd{background).
		...
		}\DIFaddend Consistent with previous studies on circadian behaviour of \emph{D. melanogaster}, populations in both LD and DD conditions exhibited strong rhythmic capture rates, with an approximate period of 24 h: 23.8 h and 23.67 h, respectively. For instance, their overall capture rate was approximately 0.6 h$^{-1}$ between ZT22 and ZT23 h, but peaked at 9.5 h$^{-1}$ between ZT01 and ZT02 h (Fig.~\ref{fig:04}\DIFdelbegin \DIFdel{D,F). 
		...
		\DIFaddend Also as hypothesised, the fly populations held in constant light (LL) showed no detectable behavioural rhythm and had a constant average capture rate of 1.6 h$^{-1}$ (sd = 0.62) (Fig.~\ref{fig:04}\DIFdelbegin \DIFdel{C,E).
		Collectively }\DIFdelend \DIFaddbegin \DIFadd{D,F).
		...
		Collectively, }\DIFaddend these observations indicate that Sticky Pis have the potential to capture circadian behaviour in a free-flying insect population.
		...
		\DIFaddend Our results corroborate a distinctive crepuscular activity pattern for male \emph{D. suzukii} and other putative drosophilids.
		\DIFaddbegin \DIFadd{For instance, 68.0\% ($CI_{95\%} =  [63.9, 71.3]$, 10,000 bootstrap replicates) of }\emph{\DIFadd{D. suzukii}} \DIFadd{and 
		...
		}\DIFaddend In contrast, Figitidae wasps were exclusively diurnal\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, with 83.0\% $CI_{95\%} =  [79.9, 85.4]$, 10,000 bootstrap replicates) of all the captures occurring during the day ($WZT < 12$), where we would expect only 50\% by chance.		
		}
		...
		\DIFadd{Together, our }\DIFaddend findings show that, even without \emph{a priori} knowledge of the diel activity of specific taxa, Sticky Pis can inform about both the community structure and temporal patterns of behaviour of a natural insect community.
        ...
        \DIFaddbegin \DIFadd{Compared to other camera-based automatic insect traps \mbox{%DIFAUXCMD
\cite{bjerge_automated_2021}}\hskip0pt%DIFAUXCMD
, we opted for a decentralised solution. 
		...
		\DIFdelbegin \DIFdel{Camera technology is quickly improving and we are currently trialling a version with superior optics (12.3 Mpx, CS mount). }\DIFdelend We found that segmentation was globally very precise (>90\%) and \DIFdelbegin \DIFdel{accurate (80}\DIFdelend \DIFaddbegin \DIFadd{sensitive (recall > 90}\DIFaddend \% for objects larger than 2~mm²). Furthermore, our machine-learning pipeline (Fig.~\ref{fig:02}) showed a high overall accuracy of the Insect Tuboid Classifier (83.1\% on average, when merging background and undefined insects, see Fig.~\ref{fig:03}). 
		\DIFaddbegin \DIFadd{Camera technology is quickly improving and our segmentation results with the new Raspberry Pi camera HQ (12.3 Mpx, CS mount) are promising, with preliminary results showing both precision and recall greater than 90\% overall, on the segmentation task.
		...
		\DIFaddbegin \DIFadd{Another potentially limiting feature of our device is the frequency of the images taken (every 20 minutes). 
		...
		Furthermore, a more frequent use of the flash light (}\emph{\DIFadd{e.g.}}\DIFadd{, every minute) may be more of a disturbance to wildlife\mbox{%DIFAUXCMD
\cite{longcore_ecological_2004}}\hskip0pt%DIFAUXCMD
.		
		}

		\DIFadd{In this respect, our image time lapse approach contrasts with continuous lighter-weight systems such as sensor-based traps, which are suited for high-frequency sampling and were recently employed to study diel activity of a single species (\mbox{%DIFAUXCMD
\cite{chen_flying_2014, goldshtein_long-term_2021}}\hskip0pt%DIFAUXCMD
).
		However, sensor-based traps are often limited to scenarios with a priori-targeted species that respond to certain specific olfactory or visual baits – which considerably narrows their applicability\mbox{%DIFAUXCMD
\cite{preti_insect_2020}}\hskip0pt%DIFAUXCMD
.
		...
		it is also essential in naive contexts, where we do not know }\emph{\DIFadd{a priori}} \DIFadd{which insects may be present.
		Indeed, physical specimens are needed for visual or DNA-based taxonomic characterisation, in particular when working on diverse or undescribed communities\mbox{%DIFAUXCMD
\cite{turney_non-repeatable_2015}}\hskip0pt%DIFAUXCMD
.
		...
		\DIFadd{An important consideration using sticky card is their potential to become saturated with insects. 
		...
		\DIFdelbegin \DIFdel{In the }\DIFdelend \DIFaddbegin \DIFadd{Importantly, any insect trap only gives a biased estimate of the number and activity on insects at a given time.
		For sticky cards, the probability of capturing an insect depends, at a minimum, on the flying activity, the size of the population and the trap attractiveness (and the ``conversion rate'' – }\emph{\DIFadd{i.e.}}\DIFadd{, the probability of trapping an insect given it is attracted). Importantly, Sticky Pis only capture mobile adult insects, and, therefore, cannot explicitly quantify the timing of important behaviours such as mating, feeding, egg laying, emergence and quiescence.
		...
		In most cases, the daily variation of population size is likely negligible. In contrast, several factors may render trap attractiveness and conversion rates variable during the day. First, visual cues – which impact a trap’s capture rates\mbox{%DIFAUXCMD
\cite{clare_pheromone_2000, sukovata_importance_2020} }\hskip0pt%DIFAUXCMD
– vary for any capture substrate (}\emph{\DIFadd{e.g.}}\DIFadd{, light intensity and spectral qualities inevitably fluctuate).
		Second, insects' ability to detect, avoid or escape traps may temporarily differ. Last, the preference for certain trap features could itself be a circadian trait. For instance, the responses of certain insects to colours\mbox{%DIFAUXCMD
\cite{lazopulo_daytime_2019} }\hskip0pt%DIFAUXCMD
allelochemicals\mbox{%DIFAUXCMD
\cite{barrozo_response_2004,eilerts_odor-specific_2018} }\hskip0pt%DIFAUXCMD
and semiochemicals\mbox{%DIFAUXCMD
\cite{rosen_circadian_2003, schendzielorz_time_2012, zhukovskaya_circadian_1995} }\hskip0pt%DIFAUXCMD
is time-modulated.
		...
		Indeed, studying the contrast in trapping rates between different, ecologically relevant, trap features (}\emph{\DIFadd{e.g.}}\DIFadd{, baits, colour and location) could help to develop new and improved trapping methodologies while bridging chronobiology and behavioural ecology.
		}

		\DIFadd{In addition to insect capture rates, Sticky Pis also monitor humidity and temperature, which are both crucial for most insects behaviour and demography \mbox{%DIFAUXCMD
\cite{jaworski_effect_2013,pawson_predicting_2017}}\hskip0pt%DIFAUXCMD
.
		...
		\paragraph{Data}
		We acquired a diverse collection of \DIFdelbegin \DIFdel{309 }\DIFdelend \DIFaddbegin \DIFadd{483 Sticky Pi }\DIFaddend images by, first, setting standalone sticky cards in the Vancouver area for one to two weeks and taking images with a Sticky Pi afterwards, and, second, by systematically sampling images from the field experiments. The first set of ‘offline’ images was “physically augmented” by taking photographs in variable conditions, which included illumination, presence of water droplets and thin dust particles. 
		\DIFaddbegin \DIFadd{In order to generalise our algorithm, we also collected another ``foreign'' 171 images of sticky cards acquired with other devices. Amongst the foreign images 140 were acquired by ourselves using the Raspberry Pi camera HQ (in 2021), and 31 were provided by the community (digital cameras and desktop scanner) -- see acknowledgements. 
		}\DIFaddend We annotated images using Inkscape SVG editor, encoding annotations as SVG paths. The outline of each visible arthropod was drawn. The contours of two adjacent animals were allowed to overlap. We automatically discarded objects smaller than 30 px (\emph{i.e.}, <~2~mm objects that are indiscernible in the images by manual annotators) or wider than 600 px (\emph{i.e.}, objects larger than 40~mm, which were generally artefacts since the vast majority of captured insects are smaller in our study region). Partial insects were only considered if their head and thorax were both visible. This procedure resulted in a total of 
		\DIFdelbegin \DIFdel{16,020 }\DIFdelend %DIF > 24,982+8574
		\DIFaddbegin \DIFadd{33,556
		}\DIFaddend segmented insects.

		\paragraph{Training}
		To perform instance segmentation, we used Mask R-CNN\cite{he_mask_2017}. In order to train the algorithm, images were pseudo-randomly split into a validation (25\%, \DIFdelbegin \DIFdel{70 }\DIFdelend \DIFaddbegin \DIFadd{96 }\DIFaddend images) and a training (75\%, \DIFdelbegin \DIFdel{239 }\DIFdelend \DIFaddbegin \DIFadd{387 }\DIFaddend images) set, based on their md5 checksum.
		\DIFdelbegin \DIFdel{Random digital augmentation was applied to all images in }\DIFdelend \DIFaddbegin \DIFadd{In order to account for partial insects on the edge of the pictures all images were zero-padded with a 32px margin.
		We performed augmentation on }\DIFaddend the training set \DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{as follow. 
		First, random regions of $1024 \times{} 1024$ px were cropped in the padded images.
		Then, we applied, to each image: random }\DIFaddend rotation (0 \DIFdelbegin \DIFdel{, 90, 180, 270 degrees), }\DIFdelend \DIFaddbegin \DIFadd{to 360 degrees); random }\DIFaddend vertical and horizontal reflections\DIFdelbegin \DIFdel{, brightness and contrast }\DIFdelend \DIFaddbegin \DIFadd{; and alterations of saturation, brightness, contrast and hue }\DIFaddend (uniform random in [0.9, 1.1]). 
...
\DIFdelend We use the detectron2 implementation \cite{wu_detectron2_2019} of Mask R-CNN to perform the instance segmentation (insect \emph{vs.} background). We retrained a ResNet50 conv4 backbone, with conv5 head, which was pre-trained on the COCO dataset, 
		\DIFdelbegin \DIFdel{for 110}\DIFdelend %DIF > fixme  N iteration?! %todo
		\DIFaddbegin \DIFadd{for 150}\DIFaddend ,000 iterations 
		(\DIFdelbegin \DIFdel{14 }\DIFdelend \DIFaddbegin \DIFadd{12 }\DIFaddend images per batch) with an initial learning rate of \DIFdelbegin \DIFdel{0.05}\DIFdelend \DIFaddbegin \DIFadd{0.002}\DIFaddend , decaying by $\gamma=0.8$ every 10,000 iterations.
		...
		\DIFadd{Formally, given a }\DIFaddend pair of objects $m, n$, in images $\mathbf{X}^i$ and $\mathbf{X}^j$, we have the binary masks $A_m$ and $A_n$ of $m$ and $n$, respectively.
		...
		\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{In order to integrate the non-linear relationships between the }\DIFaddend two resulting similarity values, $S(m,n)$ and $Q(m,n)$\DIFdelbegin \DIFdel{are then processed by a small, }\DIFdelend \DIFaddbegin \DIFadd{, as well as other descriptive variables, we used a }\DIFaddend custom, four-layers, fully connected neural network, $H(I)$. The inputs are: 
		$$
		...
		\paragraph{Data}
		In order to train the Siamese Insect Matcher core Matching function $M$, we first segmented image series from both berry field trials (see below) with the Universal Insect Detector to generate annotations (see above). We randomly sampled pairs of images from the same device, with the second image between 15 min and 12 h after the first one. We created a composite SVG image that contained a stack of the two images, and all annotations as paths. We then manually grouped (\emph{i.e.}, SVG groups) insects that were judged the same between the two frames. We generated \DIFdelbegin \DIFdel{253 }\DIFdelend \DIFaddbegin \DIFadd{397 }\DIFaddend images this way, containing a total of \DIFdelbegin \DIFdel{7238 }\DIFdelend \DIFaddbegin \DIFadd{9652 
		}\DIFaddend positive matches. Negative matches (N = \DIFdelbegin \DIFdel{154,842}\DIFdelend \DIFaddbegin \DIFadd{200,728}\DIFaddend ) were defined as all possible non-positive matches between the first and second images. Since the number of negatives was very large compared to the positive matches, we biased the proportion of negative matches to 0.5 by random sampling during training.
...
\cite{geissmann_sticky_2022}}\hskip0pt%DIFAUXCMD
		...
		\DIFaddbegin \DIFadd{Temperature and humidity data extracted from the DHT22 sensors of the Sticky Pis are reported in Supplementary Fig.~S\ref{supfig:08}.
		}\DIFaddend 
...
for making their images available 
	...
	\begin{figure}[ht]
		\centering
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1.0\linewidth]{fig/03_itc/itc.png}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.6\linewidth]{fig/03_itc/itc.png}
		\DIFaddendFL \caption{\textbf{Insect Tuboid Classifier description and performance}. \textbf{A}, Algorithm to classify insect tuboids. The first image as well as five randomly selected within the first day of data are selected. Each image is scaled and processed by a ResNet50 network to generate an output feature vector per frame. Each vector is augmented with the original scale of the object, and the element-wise median over the six frames is computed. The resulting average feature vector is processed by a last, fully-connected, layer with an output of 18 labels. 
		\textbf{B}, \DIFdelbeginFL \DIFdelFL{Confusion matrix on }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Representative examples of }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{validation dataset for the }\DIFdelendFL 18 \DIFdelbeginFL \DIFdelFL{labels}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{different classes. 
		Note that we show only one image}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{and a total of 982 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{but input }\DIFaddendFL tuboids \DIFaddbeginFL \DIFaddFL{have multiple frames}\DIFaddendFL .
        \DIFdelbeginFL \DIFdelFL{The rows show }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{All images were rescaled and padded to $224\times224$~px squares: }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{class labels as annotated in }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{input dimensions for }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{dataset}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{ResNet50.
        The added blue scale bar}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{on }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{columns show the labels predicted by the trained algorithm}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{bottom left of each tile, represents a length of 2~mm (}\emph{\DIFaddFL{i.e.}}\DIFaddFL{, 31 px)}\DIFaddendFL .
		\textbf{C}, \DIFdelbeginFL \DIFdelFL{Summary of B}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Classification performance}\DIFaddendFL , showing precision, recall and f1-score \DIFaddbeginFL \DIFaddFL{(the harmonic mean of the precision and recall) }\DIFaddendFL for each label. 
		\DIFdelbeginFL \textbf{\DIFdelFL{D}}%DIFAUXCMD
\DIFdelFL{, Representative examples of the 18 different classes, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Row }\DIFaddendFL numbers match \DIFdelbeginFL \DIFdelFL{rows }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{labels }\DIFaddendFL in B\DIFdelbeginFL \DIFdelFL{and C}\DIFdelendFL . \DIFaddbeginFL \DIFaddFL{See supplementary table 3 for the full confusion matrix.
		}\DIFaddendFL Abbreviated rows in \DIFdelbeginFL \DIFdelFL{B and }\DIFdelendFL C are \emph{Macropsis fuscula} (3), \emph{Drosophila suzukii} males (4), drosophilids that are not male \emph{D. suzukii} (5), \emph{Anthonomus rubi} (11), \emph{Psyllobora vigintimaculata} (12), Coleoptera that do not belong to any above subgroup (14) and \emph{Lasioglossum laevissimum} (16).
		}
		\label{fig:03}
	\end{figure}
	...
	\begin{figure}[ht]
		\centering
		\includegraphics[width=0.8\linewidth]{fig/06_temporal_niches/temporal_niches.png}
	...
	\begin{supfig}[ht]
		\centering
		\includegraphics[width=0.6\linewidth]{fig/S_fig_01_platform/platform.png}
	...
	\begin{supfig}[ht]
		...
		\caption{\textbf{Description of the Siamese Insect Matcher.} \textbf{A}, The matching metric in the Siamese Insect Matcher is based on a Siamese network (see Methods section). \textbf{B}. The resulting score, $M$, is used in three steps to connect insect instances between frames. The algorithm results in a series of tuboids, which are representations of single insects through time.}
		\label{supfig:03}
	\end{supfig}
	\pagebreak
	...
	\DIFaddbegin \begin{supfig}[ht]
	\centering
	\includegraphics[width=0.9\linewidth]{fig/S_fig_07_saturation/trap_saturation.png}
	\caption{\textbf{\DIFadd{Capture rates are not impacted by previously captured insects.}} \textbf{\DIFadd{A}}\DIFadd{, Cumulative number of insect over time, for each trap (black line). No noticeable reduction  of trapping rate occur, even when large number of insects (>100) are captured. }\textbf{\DIFadd{B}}\DIFadd{, Proportion of insect of a given taxa captured in the first three days of a week as a function of the final total number of all insects. The red dotted line indicate the null hypothesis: half of the insects are captured in the first half of each experiment and total number of captured insects does not affect capture rate of a given taxa. The blue lines, and their error bars are individual linear models, none of which show a significant slope or intercept, t-tests on model coefficients.}}
	\label{supfig:07}
	\end{supfig}

   \begin{supfig}[ht]
	\centering
	\includegraphics[width=0.6\linewidth]{fig/S_fig_08_temp_hum/temp_hum.png}
	\caption{\textbf{\DIFadd{Environmental conditions during both experiments.}} \DIFadd{Temperature and relative humidity throughout field experiment, as recorded by Sticky Pi's sensors (DHT 22). }\textbf{\DIFadd{A}}\DIFadd{, the experiment in the raspberry field. }\textbf{\DIFadd{B}}\DIFadd{, the experiment in the blackberry field.
	Black and white rectangles, below and above each plot show days (between sunrise and sunset) and night (between sunset and sunrise) in white and black, respectively. Sun position is inferred through the }\texttt{\DIFadd{maptools}} \DIFadd{package\mbox{%DIFAUXCMD
\cite{bivand_maptools_2020}}\hskip0pt%DIFAUXCMD
.
	...
	\textbf{Representative insect \DIFdelbegin \DIFdel{vouchers }\DIFdelend \DIFaddbegin \DIFadd{specimen }\DIFaddend used for DNA barcoding.} 
	...
	\paragraph*{\DIFadd{Supplementary Table 3}}
	\textbf{\DIFadd{Confusion matrix for the Insect Tuboid Classifier.}} 
	\DIFadd{Detailed confusion matrix for the 982 tuboids of the validation set.
	Rows and columns indicate ground truth and predicted labels, respectively.
	Abbreviated labels are }\emph{\DIFadd{Macropsis fuscula}} \DIFadd{(3), }\emph{\DIFadd{Drosophila suzukii}} \DIFadd{males (4), drosophilids that are not male }\emph{\DIFadd{D. suzukii}} \DIFadd{(5), }\emph{\DIFadd{Anthonomus rubi}} \DIFadd{(11), }\emph{\DIFadd{Psyllobora vigintimaculata}} \DIFadd{(12), Coleoptera that do not belong to any above subgroup (14) and }\emph{\DIFadd{Lasioglossum laevissimum}} \DIFadd{(16).
    }\href{https://figshare.com/s/7585a7cb174160a1c416}{\DIFadd{Data on figshare.}}
	\DIFaddend 
